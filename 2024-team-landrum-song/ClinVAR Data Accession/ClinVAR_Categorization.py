...
def search_and_save_gene_info_combined(gene, email, only_first=True):
    ...
    id_list = ",".join(ids)
    fetch_handle = Entrez.esummary(db=db, id=id_list, rettype="vcv",from_esearch="true")
    ...
    for doc_summary in root.findall(".//DocumentSummary"):
        ...
        medgen_entries = [(xref.find(".//db_source").text, xref.find(".//db_id").text) for xref in doc_summary.findall(".//trait_xref") if xref.find(".//db_source").text == "MedGen"]
        ...
        for i, rcv_id in enumerate(rcv_ids):
            medgen_db, medgen_id = medgen_entries[i] if i < len(medgen_entries) else ("Unknown", "Unknown")
    ...
    for item in data:
        ...
        MedGen_el = SubElement(RCV_el, 'MedGen', db=item['MedGen']['db'], id=item['MedGen']['id'])
